...
func GetIcingaState(db *database.Database, serviceUuid types.UUID) (IcingaState, string) {
	...
	if podCount == 0 {
		return Unknown, fmt.Sprintf("No pods associated with this service.")
	}
	...
	if allOk {
		return Ok, fmt.Sprintf("All associated pods are running.")
	} else if allCritical {
		return Critical, fmt.Sprintf("None of the associated pods are running.")
	} else if atLeastOnePodOk && atLeastOnePodCritical {
		return Warning, fmt.Sprintf("Some associated pods are not running.")
	}
